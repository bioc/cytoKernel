...
CytoK <- function(object,group_factor,lowerRho=2,
  ...
  DExpfeatures <- (.DEfeatures(pValueAdj, alpha) * 100)/dim(object)[1]
  
  results <- new("CytoK")
  if(is.null(featureColumns)) {
    results@CytoKFeatures <- data.frame(EffectSize=shrunkEffectSizeMean,
                                        EffectSizeSD=shrunkEffectSizeSD, 
                                        pvalue = pValue,padj = pValueAdj)
    } else {
    results@CytoKFeatures <- data.frame(featureColumns,
